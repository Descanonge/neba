...
class MetadataMethod:
    ...
    items
        List of items names generated by this method. If left to None, the method is
        considered "simple": it generates a single item of the same name as the method.
    name_mapping
    ...
    def generate(self, gen: MetadataGenerator) -> None:
        ...
        try:
            ...
            log.warning("Failed to run metadata method '%s' (%s)", self.name, exc)
            return
    ...
    def rename(self, name: str | None = None, **kwargs: str) -> None:
        ...
        ----------
        name
            If the method is simple, rename to that.
        kwargs
        ...
        if name is not None:
            if not self.simple:
                raise TypeError(
                    "Metadata method is not simple, cannot rename "
...
def method(
    ...
    *,
    items: list[str] | None = None,
    name_mapping: Mapping[str, str] | None = None,
) -> Callable[[Callable], MetadataMethod] | MetadataMethod:
    """Decorator that turns methods into metadata method.

    Can be used as ``@method`` or ``@method(items=..., ...)``.
    ...
    def decorator(f: Callable) -> MetadataMethod:
        elt = MetadataMethod(f, items=items, name_mapping=name_mapping)
...
class MetadataOptions(Section):
    ...
    methods = List(
        ...
        allow_none=True,
        default_value=None,
        help="Manually specify methods to run. If left to None, all methods are run.",
    )
...
class MetadataGenerator:
    ...
    def __init__(self, di: DataInterface, **kwargs: Any) -> None:
        ...
        for basetype in reversed(self.__class__.mro()):
            for key, value in basetype.__dict__.items():
                if isinstance(value, MetadataMethod):
    ...
    def get_methods(self) -> list[str]:
        ...
        methods_to_skip = []
        for selector_option, to_skip in self.options.methods_to_skip.items():
            if not getattr(self.options, selector_option):
                methods_to_skip += to_skip

        return [elt for elt in methods if elt not in methods_to_skip]

    def generate(self) -> dict[str, Any]:
        """Generate metadata."""
        for name in self.get_methods():
            if (method := self.methods.get(name, None)) is None:
                raise AttributeError(f"No metadata method named '{name}'")
    ...
    @method(items=["creation_diff_short", "creation_diff_long"])
